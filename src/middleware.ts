...
// Astro.currentLocale (set by the i18n config), falling back to English for
// routes outside /[lang]/ (only the root redirect, which doesn't read locals).
export const onRequest = defineMiddleware((context, next) => {
  const lang = (context.currentLocale ?? Languages.en) as Languages;
